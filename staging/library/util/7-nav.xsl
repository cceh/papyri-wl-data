...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/*/*:grc">
        <xsl:for-each-group select="*:div" group-by="*:entry/*:form/*:orth[@type='regularised']/substring(translate(., 'αβγδεζηθικλμνξοπρσςτυφχψω', 'ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΣΤΥΦΧΨΩ'),1,1)">
    ...
    <xsl:template match="/*/*:la">
        <xsl:for-each-group select="*:div" group-by="*:entry/*:form/*:orth[@type='regularised']/substring(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'),1,1)">
    ...
    <xsl:template name="initDuo">
        ...
        <xsl:for-each-group select="current-group()/*:entry" group-by="*:form/*:orth[@type='regularised']/substring(translate(., $lower, $upper),1,2)">
