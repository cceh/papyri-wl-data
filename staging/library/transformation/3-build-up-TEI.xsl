    ...
    <xsl:template match="*:RESULTSET/*:ROW[not(contains(*:COL[2]/*:DATA,'Leitkarte'))]">
        ...
        <entry>
            ...
            <form type="lemma">
                ...
                <xsl:choose>
                    <xsl:when test="$newEntry = 'false'">
                        <note type="resp">Anlage des Lemmas durch <name ref="editors.xml#hagedorn_d">D. Hagedorn</name> <date type="creation" notAfter="2016">vor 2016</date>.</note>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xr>
                <list>
                    <xsl:for-each select="tokenize(*:COL[8],';\s')">
                        ...
                        <item>
                            <ref target="http://papyri.uni-koeln.de/papyri-woerterlisten/quellen#{$litID}">
                                <xsl:choose>
                                    <xsl:when test="contains(.,'(')">		
                                        <xsl:value-of select="normalize-space(translate(substring-before(.,'('),'⁲','²'))"/><!-- U+2073: ³ -->		
                                    </xsl:when>		
