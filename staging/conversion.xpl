...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" xmlns:c="http://www.w3.org/ns/xproc-step" version="1.0" name="wl-input-pipeline">
    ...
    <p:choose>
        ...
        <p:otherwise>
            ...
            <!-- this should be moved to a separate pipeline and then referenced -->
            <p:documentation>
    ...
    <!-- wrap conditionally: only if no duplicates exist; else raise error -->
    <!--<p:documentation>
        <h2>...</h2>
        <p>...</p>
    </p:documentation>
    <p:xslt name="duplicate-check">
        <p:input port="stylesheet">
            <p:document href="./check-current-data-for-duplicates.xsl"/>
        </p:input>
    </p:xslt>
    ...
    <!--<p:store>
        <p:with-option name="href" select="concat('reporting/import-report_',format-date(current-date(), '[Y0001][M01][D01]'),'.xml')"/>
        <p:with-option name="indent" select="'true'"/>
    </p:store>-->
